import { useRef, useState } from "react";
import { motion } from "motion/react";
import { useTranslation } from "react-i18next";
import { Swords } from "lucide-react";
import { getIsWarning } from "../../utils/deckStatusHelper";
import PortalTooltip from "./PortalTooltip";

const richEntranceVariants = {
  hidden: {
    opacity: 0,
    scale: 0.5,
    y: 120,
    filter: "blur(8px)",
  },
  show: {
    opacity: 1,
    scale: 1,
    y: 0,
    filter: "blur(0px)",
  },
};

const richEntranceTransition = {
  duration: 0.5,
  ease: [0.22, 1, 0.36, 1],
};

const lightEntranceVariants = {
  hidden: {
    opacity: 0,
    y: 8,
  },
  show: {
    opacity: 1,
    y: 0,
  },
};

const lightEntranceTransition = {
  duration: 0.18,
  ease: "easeOut",
};

export default function DeckStatusBar({
  icon: Icon = Swords,
  value,
  limit,
  mode,
  tooltipTitleKey,
  tooltipDescriptionKey,
  showLabel = false,
  fullWidth = false,
  lightweight = false,
}) {
  const { t } = useTranslation();

  const statusBarRef = useRef(null);

  const [isTooltipVisible, setIsTooltipVisible] = useState(false);

  const warning = getIsWarning({ value, limit, mode });

  const entranceVariants = lightweight
    ? lightEntranceVariants
    : richEntranceVariants;
  const entranceTransition = lightweight
    ? lightEntranceTransition
    : richEntranceTransition;

  return (
    <>
      <motion.div
        ref={statusBarRef}
        className={fullWidth ? "w-full" : ""}
        onMouseEnter={() => setIsTooltipVisible(true)}
        onMouseLeave={() => setIsTooltipVisible(false)}
        initial="hidden"
        animate="show"
        exit="hidden"
        variants={entranceVariants}
        transition={entranceTransition}
      >
        <div
          className={`
              flex
              h-12
              items-center
              gap-3
              rounded-xl
              border
              px-4
              text-white
              shadow-lg
              cursor-default
              ${fullWidth ? "w-full" : ""}
              ${
                warning
                  ? "border-red-500/60 bg-red-950/50 shadow-red-500/15 hover:bg-red-950/30 hover:border-red-400"
                  : "border-slate-700 bg-slate-900/70 shadow-amber-500/10 hover:bg-slate-800 hover:border-amber-400"
              }
            `}
        >
          {/* ICON */}
          <div
            className={`
                flex
                items-center
                justify-center
                ${warning ? "text-red-400" : "text-amber-400"}
              `}
          >
            <Icon size={20} />
          </div>

          {/* LABEL (mobile drawer) */}
          {showLabel && (
            <span className="flex-1 truncate text-left text-sm text-slate-300">
              {t(tooltipTitleKey)}
            </span>
          )}

          {/* VALUE */}
          <div className="grid grid-cols-[2rem_auto] items-baseline gap-2">
            <motion.span
              key={value}
              initial={
                lightweight
                  ? { opacity: 0.6 }
                  : {
                      scale: 0.88,
                      y: 2,
                      opacity: 0.7,
                      filter: "drop-shadow(0 0 0px rgba(251,191,36,0))",
                    }
              }
              animate={
                lightweight
                  ? { opacity: 1 }
                  : {
                      scale: [0.88, 1.18, 1],
                      y: [2, -2, 0],
                      opacity: 1,
                      filter: warning
                        ? [
                            "drop-shadow(0 0 0px rgba(248,113,113,0))",
                            "drop-shadow(0 0 12px rgba(248,113,113,0.9))",
                            "drop-shadow(0 0 4px rgba(248,113,113,0.35))",
                          ]
                        : [
                            "drop-shadow(0 0 0px rgba(251,191,36,0))",
                            "drop-shadow(0 0 10px rgba(251,191,36,0.9))",
                            "drop-shadow(0 0 3px rgba(251,191,36,0.35))",
                          ],
                    }
              }
              transition={
                lightweight
                  ? { duration: 0.15, ease: "easeOut" }
                  : {
                      duration: 0.38,
                      times: [0, 0.45, 1],
                      ease: [0.22, 1, 0.36, 1],
                    }
              }
              className={`
                text-right
                font-cinzel
                text-xl
                font-bold
                tabular-nums
                ${warning ? "text-red-400" : "text-amber-400"}
              `}
            >
              {value}
            </motion.span>

            <span className="text-xs text-slate-400 tabular-nums">
              /{limit}
            </span>
          </div>
        </div>
      </motion.div>

      <PortalTooltip
        targetRef={statusBarRef}
        visible={isTooltipVisible}
        title={t(tooltipTitleKey)}
        description={t(tooltipDescriptionKey)}
        width="w-60"
      />
    </>
  );
}
